...
- **照片地理位置**：拍照时自动将 GPS（经纬度/海拔/时间戳）写入照片 EXIF，可在相册地图视图中查看拍摄地点；顶栏定位指示器（黄=已记录 / 白=等待定位 / 灰=未授权，点按跳设置）
- **实况照片（Live Photo）**：照片模式底部「实况」胶囊开关，开启后拍摄静态照片 + 短视频配对保存为实况照片（设备支持时显示）
- **Apple ProRAW**：照片模式底部「RAW」胶囊开关，开启后以 ProRAW（DNG）格式拍摄，保留最大后期空间；与实况互斥
...
| `NSLocationWhenInUseUsageDescription` | 定位使用说明（写入照片 GPS） |
